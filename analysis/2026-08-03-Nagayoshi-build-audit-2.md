...
- **Options tab:** My numbers are different by design, not by mistake. My strike is at spot — it protects me if the rate drops, and I take the market rate if it goes up. In the collar version, the sold call caps my upside instead — no "better market rate" there. The class example sets the strike far enough away that it never gets used in their scenarios, so they pay the premium but get no real protection.
- **Color legend:** The legend text existed, but the cells didn't actually use fill colors. Added real fills — yellow for input, blue for assumption, gray for formula, green for cross-tab link.
- **Open item:** `FC_AMT` ambiguity is still open. I derived €3,947,368 by translating a stated "$4,500,000 receivable" at spot. This is the single largest open assumption in the model. Carrying it into Stage 4 as the first line item — every USD figure in the workbook scales with it.
...
- `FC_AMT` ambiguity — still open. €3,947,368 is derived by translating the stated $4,500,000 at spot. Carry into Stage 4 as the first line item.
